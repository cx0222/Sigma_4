...
ast, _same_, check = False, False, True
OS, ver, time0 = 'Windows', '4.0', datetime.datetime.now()
...
def _get_(func):
    ...
    if func == 1:
        ...
        outcut = listcs[1]
        window3 = tk.Toplevel()
        window3.title('三元轮换对称不等式运算程序  ver 4.0  SOS分拆结果')
    ...
    if func == 2:
        window4 = tk.Toplevel()
        out0 += sc3_0(exp0)
        window4.title('三元轮换对称不等式运算程序  ver 4.0  Schur分拆结果')
...
def _catch_(func):
    ...
    if func == 1:
        ...
        out0 += '\n[结果] ' + str(pqr2_6(exp0))
        window5.title('三元轮换对称不等式运算程序  ver 4.0  pqr分拆结果')
    if func == 2:
        if as_ast:
            out0 += '\n[整理] ' + str(normal0(exp0))
        else:
        ...
        listast = ast3_0(exp0)
        out0 += '\n[结果] ' + listast[0]
        window5.title('三元轮换对称不等式运算程序  ver 4.0  差分代换结果')
...
def _further_(out):
    ...
    window7.title('三元轮换对称不等式运算程序  ver 4.0  进一步运算')
...
def _more_(list0):
    ...
    window8.title('三元轮换对称不等式运算程序  ver 4.0  进一步运算')
...
def _exit_():
    ...
    with open(file_8, 'a') as file_8_0:
        file_8_0.write('Sigma 4.0 正常关闭！时间：')
        file_8_0.write('{0}\n\n\n'.format(time2))
    window0.quit()
    window0.destroy()
    exit()
...
def _about_():
    ...
    bg0 = ImageTk.PhotoImage(Image.open('ver4_0.gif'))
...
def _report_():
    window9 = tk.Toplevel()
    window9.title('三元轮换对称不等式运算程序  ver 4.0 [20210220] 反馈系统')
    window9.geometry('800x600')
    window9.resizable(True, True)
    bg0 = ImageTk.PhotoImage(Image.open('ver4_0_1.gif'))
    ...
    title3 = tk.Label(window9,
                      text='感谢您的反馈！',
                      font=('Arial', 18))
    report1 = tk.Button(window9, text='发送至官方邮箱', command=lambda: _sent_(entry1.get(), entry2.get()))

    canvas0.pack(), title9_1.pack(), entry1.pack(), title2.pack(), entry2.pack(), title3.pack(), report1.pack()
    window9.mainloop()
...
window0 = Tk()
window0.title('三元轮换对称不等式运算程序  ver 4.0 [20210220]')
...
title0 = tk.Label(canvas1,
                  text='\n基于Python 3开发的 三元轮换对称不等式运算程序（发行版）'
                       '\n ver 4.0 [20210220]  cx0222 Jinan, Shandong',
                  font=('Arial', 18))
title0.place(relheight=0.15, relwidth=0.8, relx=0.1, rely=0)
title1 = tk.Label(canvas1,
                  text='请输入不等式，并以a, b, c作为不等式的变量',
                  font=('Arial', 13))
title1.place(relheight=0.15, relwidth=0.8, relx=0.1, rely=0.27)
...
menu_1.add_command(label='关于Sigma 4.0', command=_about_)
menu_1.add_command(label='发行说明及开发信息', command=_all_)
menu_1.add_command(label='反馈给开发者', command=_report_)
menu_1.add_separator()
menu_1.add_command(label='退出Sigma 4.0', command=_exit_)
menu0.add_cascade(label='说明与反馈', menu=menu_1)

about0 = tk.Button(canvas1, text='关  于', command=_about_)
about0.place(relheight=0.07, relwidth=0.15, relx=0.3, rely=0.15)
report0 = tk.Button(canvas1, text='反  馈', command=_report_)
report0.place(relheight=0.07, relwidth=0.15, relx=0.55, rely=0.15)
entry0 = tk.Entry(canvas1)
entry0.place(relheight=0.07, relwidth=0.80, relx=0.1, rely=0.24)
cs0 = tk.Button(canvas1, text='SOS分拆', command=lambda: _get_(1))
cs0.place(relheight=0.07, relwidth=0.15, relx=0.3, rely=0.38)
sc0 = tk.Button(canvas1, text='Schur分拆', command=lambda: _get_(2))
sc0.place(relheight=0.07, relwidth=0.15, relx=0.3, rely=0.48)
pqr0 = tk.Button(canvas1, text='pqr分拆', command=lambda: _catch_(1))
pqr0.place(relheight=0.07, relwidth=0.15, relx=0.3, rely=0.58)
ast0 = tk.Button(canvas1, text='差分代换', command=lambda: _catch_(2))
ast0.place(relheight=0.07, relwidth=0.15, relx=0.3, rely=0.68)
prove0 = tk.Button(canvas1, text='机器证明', command=lambda: _pre_(3))
prove0.place(relheight=0.07, relwidth=0.15, relx=0.3, rely=0.78)
note0 = tk.Button(canvas1, text='说  明', command=_all_)
note0.place(relheight=0.07, relwidth=0.15, relx=0.3, rely=0.88)
exit0 = tk.Button(canvas1, text='退  出', command=_exit_)
exit0.place(relheight=0.07, relwidth=0.15, relx=0.55, rely=0.88)
...
pqr2 = tk.Checkbutton(canvas1, text='是否为齐次不等式', command=lambda: _true_(6))
...
cs1.pack(), sc1.pack()
pqr1.pack(), pqr2.pack(), ast1.pack(), ast2.pack()
prove1.pack()
window0.mainloop()
